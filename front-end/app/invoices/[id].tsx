...
import { useGetInvoice, useInvalidateInvoice } from "../../hooks/useGetInvoice";
...
function getStatusLabel(status: InvoiceDetail["status"]) {
  switch (status) {
    case "paid":
      return "Payée";
    case "sent":
...
export default function InvoiceDetailScreen() {
  ...
  const { data: invoice, isLoading, isError, refetch } = useGetInvoice(Number(id));
  ...
  return (
    ...
    >
      ...
      <View className="px-6 pt-14 pb-4 flex-row items-center gap-4">
        ...
        <Chip color={getStatusColor(invoice.status)} variant="soft" size="sm">
          <Chip.Label>{getStatusLabel(invoice.status)}</Chip.Label>
        </Chip>
      </View>
        ...
        {invoice.status === "to_send" && (
          <>
            ...
            <Button
              ...
              onPress={() =>
                setPendingAction({
                  fn: () => cancelInvoice(invoice.id),
                  label: "Facture annulée",
                  confirmLabel: "Annuler la facture",
                  description:
                    "Voulez-vous annuler cette facture ? Cette action est irréversible.",
                })
              }
            >
        ...
        {invoice.status === "sent" && (
          <View className="flex-row gap-3">
            <Button
              ...
              onPress={() =>
                setPendingAction({
                  fn: () => payInvoice(invoice.id),
                  label: "Facture payée",
                  confirmLabel: "Marquer payée",
                  description:
                    "Voulez-vous marquer cette facture comme payée ?",
                })
              }
            >
              <Button.Label className="text-white font-bold">
            ...
            <Button
              className="flex-1 rounded-2xl h-12 bg-orange-500"
              onPress={() =>
                setPendingAction({
                  fn: () => markInvoiceOverdue(invoice.id),
                  label: "Facture en retard",
                  confirmLabel: "Marquer en retard",
                  description:
                    "Voulez-vous marquer cette facture comme en retard de paiement ?",
                })
              }
            >
              <Button.Label className="text-white font-bold">
        ...
        <Button
          ...
          onPress={() =>
            download({
              ...
              id: invoice.id,
              filename: `${invoice.invoiceNumber || `FAC-${invoice.id}`}.pdf`,
