      ...
      <View className="p-6 pt-12">
        ...
        <View className="flex-row flex-wrap gap-4 mb-8">
          {stats.map((stat, index) => (
            <Surface key={index} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex-1 min-w-[45%]">
              ...
              <Text className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">{stat.label}</Text>
              <Text className="text-xl font-black text-gray-900 mb-2">{stat.value}</Text>
              <View className="flex-row items-center">
                <Ionicons name="arrow-up" size={12} color={stat.trendColor} />
                <Text style={{ color: stat.trendColor }} className="text-xs font-bold ml-1">{stat.trend} ce mois</Text>
              </View>
            </Surface>
          ))}
        </View>
        ...
        <Surface className="bg-blue-600 p-6 rounded-3xl shadow-xl mb-8 flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className="text-white text-lg font-black">Créer un nouveau devis</Text>
          </View>
            ...
            <Card.Body className="p-4 flex-row items-center gap-4">
              ...
              <View className="flex-1">
                <View className="flex-row justify-between items-center mb-1">
                  <Text className="font-black text-gray-900">{activity.title}</Text>
                  <Text className="font-bold text-blue-600">{activity.amount}</Text>
                </View>
